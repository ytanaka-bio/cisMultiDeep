...
Human_peak <- read.table("Human_atac_600_input.csv",sep=",",header=T,row.names=1,nrow=1,check.names=F)
Human_peak <- colnames(Human_peak)
Macaque_peak <- read.table("Macaque_atac_600_input.csv",sep=",",header=T,row.names=1,nrow=1,check.names=F)
Macaque_peak <- colnames(Macaque_peak)
Marmoset_peak <- read.table("Marmoset_atac_600_input.csv",sep=",",header=T,row.names=1,nrow=1,check.names=F)
Marmoset_peak <- colnames(Marmoset_peak)
Mouse_peak <- read.table("Mouse_atac_600_input.csv",sep=",",header=T,row.names=1,nrow=1,check.names=F)
Mouse_peak <- colnames(Mouse_peak)
...
write.table(Mouse_atac_bed[intersect(rownames(Mouse_atac_bed),Mouse_peak),],"Mouse_atac_600.bed",sep="\t",quote=F,row.names=F,col.names=F)
write.table(Human_atac_bed[intersect(rownames(Human_atac_bed),Human_peak),],"Human_atac_600.bed",sep="\t",quote=F,row.names=F,col.names=F)
write.table(Macaque_atac_bed[intersect(rownames(Macaque_atac_bed),Macaque_peak),],"Macaque_atac_600.bed",sep="\t",quote=F,row.names=F,col.names=F)
write.table(Marmoset_atac_bed[intersect(rownames(Marmoset_atac_bed),Marmoset_peak),],"Marmoset_atac_600.bed",sep="\t",quote=F,row.names=F,col.names=F)
